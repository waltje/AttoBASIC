...
ZIP3=${FNPFX}_APP.ZIP		# Applications and Utils
ZIP4=${FNPFX}_USB.ZIP		# Mega32U4/USB1286 USB Bootloader and Serial I/O Support Source
...
for DIR in _Bootloaders _USB_Drivers _Serial-IO  AVR_Specific_Builds ; do
...
#Build Application, Docs and Utils Files
echo "${YEL}Generating archive ${BLU}${ZIP3}${YEL}... Please wait...${NORMAL}"
# Scan for files to exclude from this archive
echo > ${TMPFILE}		# no files to exclude from this archive
$ZIPPER $ZIPOPT ${DESTDIR}/${ZIP3} _Applications _Docs _Utils >> ${LOGFILE} 2>&1 ;
if [ $? == 0 ] ; then
   echo "Completed, no errors."
else
   echo "There was a fatal error!" ;
...
echo "${YEL}Generating archive ${BLU}${ZIP4}${YEL}... Please wait...${NORMAL}"
echo > ${TMPFILE}		# no files to exclude from this archive
$ZIPPER $ZIPOPT ${DESTDIR}/${ZIP4} _Bootloaders _Serial-IO _USB_Drivers >> ${LOGFILE} 2>&1 ;
if [ $? == 0 ] ; then
   echo "Completed, no errors."
else
   echo "There was a fatal error!" ;
