...
// setup
void usb_serial_init(void);		// initialize everything
void usb_shutdown(void);                // shutdown and disconnect USB
...
#define USB_SERIAL_CONTROLS		1		//"1" to enable control code, "0" to disable
#define USB_SERIAL_BUFFERED		0		//"1" to enable buffer xfer code, "0" to disable
;
...
#define MSB(n) ((n >> 8) & 255)
  ...
  #define PRR_CONFIG() (PRR1 &= ~(1<<PRUSB))
  #define HW_CONFIG()
  #define PLLCSR_CONFIG() (PLLCSR = ((1<<PLLE)|(1<<PLLP0)))
  ...
  #define PLLCSR_CONFIG() (PLLCSR = 0x1A)
  ...
  #define USB_CONFIG() (USBCON = ((1<<USBE)|(1<<OTGPADE)))
  #define USB_FREEZE() (USBCON = ((1<<USBE)|(1<<FRZCLK)))
  #if F_USB == 16000000		// 16MHz clock source
   #define PLLCSR_CONFIG() (PLLCSR = ((1<<PINDIV)|(1<<PLLE)))
   #define PLLFRQ_CONFIG() (PLLFRQ = ((0<<PINMUX)|(1<<PLLUSB)|(0x0A<<PDIV0)))
  #elif F_USB == 8000000	// 8MHz clock source
   #define PLLCSR_CONFIG() (PLLCSR = ((0<<PINDIV)|(1<<PLLE)))
   #define PLLFRQ_CONFIG() (PLLFRQ = ((0<<PINMUX)|(0<<PLLUSB)|(4<<PDIV0)))
  #else
   #error Illegal USB Clock!
  #endif
  ...
  #define USB_CONFIG() (USBCON = ((1<<USBE)|(1<<OTGPADE)))
  #define USB_FREEZE() (USBCON = ((1<<USBE)|(1<<FRZCLK)))
  #define PLLFRQ_CONFIG()
  #if F_USB == 16000000		// 16MHz clock source
   #define PLLCSR_CONFIG() (PLLCSR = ((5<<PLLP0)|(1<<PLLE)))	//0x16
  #elif F_USB == 8000000	// 8MHz clock source
   #define PLLCSR_CONFIG() (PLLCSR = ((3<<PLLP0)|(1<<PLLE)))	//0x0E)
  #else
   #error Illegal USB Clock!
  #endif
#endif
