...
/** Initialize (clear) a queue (word-sized items)
 *
 * @param[in] size - queue size in words
 * @param[in] pQ - pointer to queue
 *
 * NOTE *** THIS ROUTINE EXPECTS that the queue at _rbq_buf is one MORE byte than
 *          the size passed!! The standard circular FIFO algorithm requires
...
/** Atomically add word to a queue
 *
...
/** Atomically dequeue word from head of queue
 *
 * @param[in] pQ - pointer to queue
 *
 * @return uint16 - word at head of queue, or SREG[C]=1 if no data available
 */
uint16_t dQw(rbq *pQ, bool peek);

#define peekQ(pQ) dQ((pQ), TRUE)
#define peekQw(pQ) dQw((pQ), TRUE)
#define deQ(pQ) dQ((pQ), FALSE)
#define deQw(pQ) dQw((pQ), FALSE)
