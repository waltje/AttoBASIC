...
if [ -e $FNPFX.asm ] ; then
   ...
   (wineconsole --backend=curses "avrasm2.exe" -V -v1 -S "labels.tmp" -I Include/ -fI -W+ie -C V3 $1 $2 $3 $4 $5 \
   -o "$FNPFX.hex" -d "$FNPFX.obj" -e "$FNPFX.eep" -m "$FNPFX.map" -l "$FNPFX.lst" "$FNPFX.asm" \
